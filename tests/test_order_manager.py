...
class TestOrderManager:
    ...
    @pytest.fixture
    def live_order_manager(self, mock_dependencies, mock_exchange_service):
    ...
    @pytest.fixture
    def backtest_order_manager(self, mock_dependencies):
    ...
    def test_execute_order_no_grid_cross(self, backtest_order_manager, mock_dependencies, mock_exchange_service):
        mock_dependencies['grid_manager'].detect_grid_level_crossing.return_value = None
        
        backtest_order_manager.execute_order(OrderType.BUY, 1000, 900, "2024-01-01T00:00:00Z")
    ...
    def test_execute_buy_order_with_valid_grid_cross(self, bactest_order_manager, mock_dependencies):
        grid_level = Mock()
        ...
        bactest_order_manager.execute_order(OrderType.BUY, 1000, 900, "2024-01-01T00:00:00Z")
    ...
    def test_execute_sell_order_with_valid_grid_cross(self, bactest_order_manager, mock_dependencies):
        grid_level = Mock()
        ...
        bactest_order_manager.execute_order(OrderType.SELL, 1000, 1100, "2024-01-01T00:00:00Z")
    ...
    def test_execute_take_profit(self, bactest_order_manager, mock_dependencies):
        mock_dependencies['balance_tracker'].crypto_balance = 5

        bactest_order_manager.execute_take_profit_or_stop_loss_order(2000, "2024-01-01T00:00:00Z", take_profit_order=True)
    ...
    def test_execute_stop_loss(self, bactest_order_manager, mock_dependencies):
        mock_dependencies['balance_tracker'].crypto_balance = 5

        bactest_order_manager.execute_take_profit_or_stop_loss_order(1500, "2024-01-01T00:00:00Z", stop_loss_order=True)

        mock_dependencies['balance_tracker'].sell_all.assert_called_once_with(1500)
        mock_dependencies['order_book'].add_order.assert_called_once()

    def test_process_buy_order_insufficient_balance(self, bactest_order_manager, mock_dependencies):
        grid_level = Mock()
        ...
        bactest_order_manager._process_buy_order(grid_level, 1000, "2024-01-01T00:00:00Z")

        mock_dependencies['balance_tracker'].update_after_buy.assert_not_called()

    def test_process_sell_order_insufficient_crypto(self, bactest_order_manager, mock_dependencies):
        grid_level = Mock()
        buy_grid_level = Mock()
        mock_dependencies['grid_manager'].find_lowest_completed_buy_grid.return_value = buy_grid_level
        mock_dependencies['transaction_validator'].validate_sell_order.side_effect = InsufficientCryptoBalanceError

        bactest_order_manager._process_sell_order(grid_level, 1000, "2024-01-01T00:00:00Z")

        mock_dependencies['balance_tracker'].update_after_sell.assert_not_called()

    def test_process_buy_order_grid_level_not_ready(self, bactest_order_manager, mock_dependencies):
        grid_level = Mock()
        grid_level.can_place_buy_order.return_value = False

        with pytest.raises(GridLevelNotReadyError):
            bactest_order_manager._place_order(grid_level, OrderType.BUY, 1000, 5, "2024-01-01T00:00:00Z")

    def test_multiple_buy_orders_for_sell(self, bactest_order_manager, mock_dependencies):
        grid_level = Mock()
    ...
    def test_zero_quantity_order(self, bactest_order_manager, mock_dependencies):
    ...
    def test_extreme_price_fluctuation(self, bactest_order_manager, mock_dependencies):
        grid_level = Mock()

        mock_dependencies['grid_manager'].detect_grid_level_crossing.side_effect = [None, 5000]
        mock_dependencies['grid_manager'].get_grid_level.return_value = grid_level

        bactest_order_manager.execute_order(OrderType.BUY, 5000, 1000, "2024-01-01T00:00:00Z")
        ...
        bactest_order_manager.execute_order(OrderType.BUY, 5000, 4000, "2024-01-01T00:01:00Z")
    ...
    def test_negative_price_handling(self, bactest_order_manager, mock_dependencies):
        grid_level = Mock()
        mock_dependencies['grid_manager'].detect_grid_level_crossing.return_value = None

        # Simulate a negative price scenario
        bactest_order_manager.execute_order(OrderType.BUY, -1000, 1000, "2024-01-01T00:00:00Z")
        
        mock_dependencies['transaction_validator'].validate_buy_order.assert_not_called()
        mock_dependencies['balance_tracker'].update_after_buy.assert_not_called()

    def test_partial_crypto_balance_for_sell(self, bactest_order_manager, mock_dependencies):
        grid_level = Mock()
        buy_order = Mock(quantity=5)
    ...
    def test_minimal_price_difference(self, bactest_order_manager, mock_dependencies):
        grid_level = Mock()
        mock_dependencies['grid_manager'].detect_grid_level_crossing.return_value = None

        # Test minimal price difference (e.g., only $1 difference)
        bactest_order_manager.execute_order(OrderType.BUY, 1000, 999, "2024-01-01T00:00:00Z")
        
        mock_dependencies['grid_manager'].get_grid_level.assert_not_called()
        mock_dependencies['transaction_validator'].validate_buy_order.assert_not_called()
    
    def test_high_volume_trades(self, bactest_order_manager, mock_dependencies):
        grid_level = Mock()
        mock_dependencies['grid_manager'].detect_grid_level_crossing.return_value = 1000
        mock_dependencies['grid_manager'].get_grid_level.return_value = grid_level
